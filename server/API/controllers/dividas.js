...
exports.dividas_ativas_inativas = (req, res, next) => {
  ...
  Divida.find({paga: estado}) // vai buscar as dividas com flag especificada no estadp
    ...
    .then((dividas) => {
      ...
      if(!estado){
            if(admin){
              return res.json('dividastotaisadmin',{dividas : dividas.map(divida =>{
              return divida;
              })}); 
            }
            else{
              return res.json('dividastotais',{dividas : dividas.map(divida =>{
                return divida;
              })}); 
            }        
      }
      else{
        return res.json('historico',{dividas : dividas.map(divida =>{
        return divida;
      })}); 
      }
