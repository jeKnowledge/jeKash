...
exports.get_all_jekers = (req, res, next) => {
    ...
    Jeker.find().select("nome departamento").exec()
    .then(docs => {
        // OUTPUT
        const response = {
            count: docs.length, // Numero total de jekers
            Jekers: docs.map(doc => { // map cria um array com as informações seguintes de cada jeker
                return{ // Return da informação dos jekers
                    nome: doc.nome,
